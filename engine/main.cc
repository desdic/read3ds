...
void TerminateApplication(void)												// Terminate The Application
{
	...
	{
        cout << "SDL_QUIT event can't be pushed: " << SDL_GetError() << endl;		// And Eventually Report Errors
		exit(1);															// And Exit
	}
...
void ToggleFullscreen(void)													// Toggle Fullscreen/Windowed (Works On Linux/BeOS Only)
{
	...
	{
		cout << "Unable to toggle fullscreen: " << SDL_GetError() << endl;			// We're Reporting The Error, But We're Not Exiting
	}
...
int main(int argc, char **argv)												// Our Main Funcion!
{
	...
	{
        cout << "Unable to open SDL: " << SDL_GetError() << endl;					// If SDL Can't Be Initialized
	...
	{
		cout << "Unable to open screen surface: " << SDL_GetError() << endl;		// If Something's Gone Wrong, Report
		exit(1);															// And Exit
	}

	SDL_WM_SetCaption("Test", NULL);										// We're Setting The Window Caption

	if(!InitTimers(&LastCount))												// We Call The Timers Init Function
	{
        cout << "Can't init the timers: " << SDL_GetError() << endl;				// If It Can't Init, Report
		exit(1);															// And Exit
	}

	if(!InitGL(Screen))														// We're Calling The OpenGL Init Function
	{
		cout << "Can't init GL: %s\n" << SDL_GetError() << endl;						// If Something's Gone Wrong, Report
		exit(1);															// And Guess What? Exit
	}

	if(!Initialize())														// Now We're Initting The Application
	{
		cout << "App init failed: " << SDL_GetError() << endl;						// Blah Blah Blah, Blah
		exit(1);															// And Blah
	}
